...
function getAngleCache() {
    ...
    // Generate cache for all rotation splits from 1 to maxSplit
    for (i = 1; i <= maxSplit; ++i) {
        radianStep = (2 * Math.PI) / i;

        for (j = 0; j < i; ++j) {
            // Use exact angle calculation instead of rounded step
            angle = Math.round((360 * j) / i);
    ...
    // Add additional commonly used angles to ensure comprehensive coverage
    const commonAngles = [
    ...
    // Add any missing common angles
    for (const commonAngle of commonAngles) {
        if (!result.has(commonAngle)) {
            radianAngle = (commonAngle * Math.PI) / 180;
            memSeg[0] = Math.sin(radianAngle);
            memSeg[1] = Math.cos(radianAngle);
            result.set(commonAngle, memSeg.slice());
        }
    }
