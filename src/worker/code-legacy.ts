/**
 * Legacy (JavaScript / `interpreterVersion < 1`) Code atom executor.
 *
 * This file is the pre-TypeScript implementation of the Code atom worker
 * path. It exposes the classic wrapped* helpers (`Move`, `Chamfer`,
 * `CutAssembly`, ...) and the `userLib` abstraction to user code. New
 * (TypeScript) Code atoms do NOT go through this file — they are routed via
 * `code.ts` which implements a cleaner surface. Keep this file focused on
 * backwards compatibility; do not add new features here.
 */
 ...
 */
export type RealizedAssembly = RealizedLeaf | RealizedBranch;
...
/**
 * Executes the given JavaScript user code with the provided arguments list.
...
export async function realizeAssembly(
...
export function isRealizedLeaf(node: RealizedAssembly): node is RealizedLeaf {
  return (
