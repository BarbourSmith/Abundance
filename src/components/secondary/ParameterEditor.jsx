...
export default (function ParamsEditor({
  ...
  useEffect(() => {
    // Only run when an atom is selected
    if (activeAtom) {
      // Ensure panel is expanded
      setCollapsed(false);
      
      // Use a significant timeout to ensure DOM is fully updated
      const focusTimeoutId = setTimeout(() => {
        try {
          // Target the most specific input field in the first row that's not disabled
          // This is typically where the first parameter input would be located
          const firstRowInput = document.querySelector(
            '.paramEditorDiv .leva__panel .leva__row:first-child input:not([disabled]), ' + 
            '.paramEditorDivRun .leva__panel .leva__row:first-child input:not([disabled])'
          );
          
          if (firstRowInput) {
            // Only focus - don't click or do extra manipulation
            firstRowInput.focus();
            
            // Select text for text/number inputs
            if (firstRowInput.type === "text" || firstRowInput.type === "number") {
              firstRowInput.select();
            }
          }
        } catch (err) {
          // Silent fail - don't disrupt the application flow if something goes wrong
        }
      }, 500); // Use a longer timeout to ensure DOM is fully rendered
